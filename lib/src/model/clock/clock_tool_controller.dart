...
class ClockToolController extends Notifier<ClockState> {
  ...
  /// Start delay remaining when the clock was last paused, carried over on
  /// [resume].
  ...
  void pause() {
    // Preserve any in-progress start delay so it carries over (and stays
    // visible) when resuming.
  ...
  void resume() {
    ...
    if (active != null && (initialOfActive.inMilliseconds != 0 || hasActiveMoved)) {
      // Carry over the delay that was left when we paused: if a delay was still
      // counting down it continues from where it stopped, otherwise the main
      // clock resumes immediately.
