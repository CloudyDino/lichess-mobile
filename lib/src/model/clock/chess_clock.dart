...
class ChessClock {
  ...
  void start({Duration? delay}) {
    ...
    // Cancel any in-progress main tick so the clock does not keep counting down
    // while the start delay is running (e.g. after switching sides).
    _timer?.cancel();
  ...
  /// Pauses the clock, returning any in-progress start delay so it can be
  /// carried over when the clock is resumed.
  ///
  /// Returns the remaining start delay, or [Duration.zero] if none was running.
  Duration pause() {
  ...
  /// Cancels all timers and marks the clock as not running.
  void _halt() {
    _stopwatch.stop();
    _startDelayTimer?.cancel();
    _timer?.cancel();
